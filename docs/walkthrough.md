...
If metadata is missing or malformed, Shadowbox falls back to numeric behavior. The only non-metadata exception is enums published explicitly by RNBO as a value list. An enum whose complete advertised choices are exactly `Off` and `On` is shown as an inline switch in the parameter browser and toggles without opening the enum chooser. On the touch display, its adjacent MIDI icon opens the parameter inspector for MIDI learn or clear without changing the toggle. Shadowbox preserves and sends the original advertised strings. Other enums, including arbitrary two-choice enums, continue to use the chooser.
