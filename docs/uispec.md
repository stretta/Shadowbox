...
- The `ShadowGrain` Ring Buffer surface requires the five parameters `Rate`, `Position`, `GrainDuration`, `Transpose`, and `RecordToggle`, plus `itriggeroverview`/`overviewchunks` and `getrecordsync`/`recordsync` message pairs. Opening the surface requests one overview and one record-phase value; subsequent record-phase updates are event-driven, and the display projects the ten-second phase locally while recording instead of polling RNBO
- Ring Buffer presents `RecordToggle` as a two-state `RECORD`/`STOP` button, keeps all five parameter values visible, and maps waveform touches through the rotated display phase back to the physical `Position` parameter. It assembles 32 one-based chunks of 50 min/max values into 800 columns; malformed or missing chunks remain visibly incomplete rather than being interpreted as waveform data
