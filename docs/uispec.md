...
- The ListVelSequencer surface binds eight ordered velocity-list rows to `1row` through `8row`, requires the corresponding `1map` through `8map` pitch parameters and `1mute` through `8mute` mute parameters, shows each row's current pitch beside its row number, and provides a compact `M` button for muting each row
