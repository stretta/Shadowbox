...
- On the ListSequencer surface, keypad digits enter numbers, `+` enters a space, `.` deletes, `-` toggles the current sign, `Enter` sends, and `/` / `*` select the previous / next field. Keypad events are ignored on every other Shadowbox screen.
